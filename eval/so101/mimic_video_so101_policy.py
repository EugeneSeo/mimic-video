"""mimic-video SO-101 EE policy wrapper for offline and server-side evaluation.

The wrapper follows the SO-101 end-effector observation/action convention:

    observation/images/hstack or front+wrist -> workspace_rgb (480x640 hstack)
    observation/state                        -> obs/lowdim_concat, shape (1, 10)
    actions                                  -> action/lowdim_concat, shape (H, 10)

The 10D state/action layout is ``[eef_pos, eef_rot_6d, gripper]``.

For server-side evaluation, prompt embeddings can be passed directly to the video
pipeline so the heavyweight T5 text encoder does not need to be loaded.
"""
...
from data_preprocessing.video.process_so101_two_camera_video import compose_two_view_frame  # noqa: E402
...
class MimicVideoSO101Policy:
    """Loads mimic-video VAM components and predicts SO-101 10D EE action chunks."""
    ...
    def __init__(self, cfg: SO101MimicVideoPolicyConfig) -> None:
        ...
        self.image_history: collections.deque[np.ndarray] = collections.deque(maxlen=self.obs_image_horizon)
        self.pipeline = self._load_pipeline()
        action_config = self.data_config.policy_io.policy_io.action.eef_pos_lowdim
    ...
    def _load_resolved_config(self):
        data_dir = self.cfg.data_dir or pathlib.Path("/tmp/mimic_video_so101_ee_stats_only")
        os.environ["SO101_EE_DATA_DIR"] = str(data_dir)
    ...
    def _infer_action_delta_mode(self) -> str:
        if "so101_ee" in self.cfg.experiment_name:
            return "relative_ee"
    ...
    def predict_from_observation(self, observation: dict[str, object], *, seed: int) -> dict[str, np.ndarray | float]:
        """Predict a SO-101 EE action chunk from an OpenPI-style observation dict."""
        started_at = time.perf_counter()
        input_vid = self._online_workspace_video_tensor(
            observation,
    ...
    def _online_workspace_video_tensor(self, observation: dict[str, object], *, reset_history: bool) -> torch.Tensor:
        if reset_history:
            self.reset_history()
        frame = self._preprocess_workspace_frame(observation)
        ...
        video = np.array(np.stack(frames, axis=1), copy=True)  # C,T,H,W in [-1, 1]
        return torch.as_tensor(video, device=self.cfg.device, dtype=self.cfg.dtype).unsqueeze(0)

    def _resolve_hstack_hwc(self, observation: dict[str, object]) -> np.ndarray:
        hstack = observation.get("observation/images/hstack")
        if hstack is not None:
            return self._to_uint8_hwc(hstack)
        front = observation.get("observation/images/front")
        wrist = observation.get("observation/images/wrist")
        if front is None or wrist is None:
            raise KeyError(
                "Observation must include observation/images/hstack or both "
                "observation/images/front and observation/images/wrist."
            )
        return compose_two_view_frame(
            self._to_uint8_hwc(front),
            self._to_uint8_hwc(wrist),
            height=self.image_height,
            width=self.image_width,
        )

    def _preprocess_workspace_frame(self, observation: dict[str, object]) -> np.ndarray:
        from PIL import Image

        array = self._resolve_hstack_hwc(observation)
    ...
    @staticmethod
    def _to_uint8_hwc(image: object) -> np.ndarray:
    ...
    def _online_state_tensor(self, state: object) -> torch.Tensor:
        ...
        if state_array.shape != (1, self.action_dim):
            raise ValueError(
                f"Expected observation/state shape ({self.action_dim},) or (1, {self.action_dim}), "

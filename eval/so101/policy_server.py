"""Prediction server for mimic-video SO-101 EE action chunks."""
...
class SO101PolicyServer:
    ...
    def metadata(self) -> dict[str, Any]:
        ...
        metadata.update(
            {
                "server": "mimic-video-so101-ee-prediction",
                "host": self.args.host,
                "port": self.args.port,
                "payload_protocol": PAYLOAD_PROTOCOL,
                "transport": TCP_TRANSPORT if self.args.transport == "tcp" else "websocket",
                "observation_keys": [
                    "observation/images/hstack or observation/images/front + observation/images/wrist",
                    "observation/state",
    ...
    def handle_request(self, request: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "metadata": {
                ...
                "views": self.policy.metadata()["views"],
            },
        }
...
def serve_tcp(args: ServerArgs) -> None:
    ...
    print(f"Serving mimic-video SO-101 EE policy on tcp://{args.host}:{args.port}", flush=True)
...
def serve_websocket(args: ServerArgs) -> None:
    ...
    print(f"Serving mimic-video SO-101 EE policy on ws://{args.host}:{args.port}", flush=True)

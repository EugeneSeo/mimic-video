"""Small raw client for smoke-testing the mimic-video SO-101 EE server."""
...
def parse_args() -> ClientArgs:
    ...
    parser.add_argument("--action-dim", type=int, default=ClientArgs.action_dim)
    ...
    parser.add_argument(
        ...
        default=True,
        help="Send a single hstack image (default) or separate front/wrist views.",
    )
...
def build_observation(args: ClientArgs, rng: np.random.Generator, request_idx: int) -> dict[str, Any]:
    ...
    if args.use_hstack:
        ...
        observation["observation/images/hstack"] = image
    else:
        half_w = args.image_width // 2
        observation["observation/images/front"] = rng.integers(
            0, 256, size=(args.image_height, half_w, 3), dtype=np.uint8
        )
        observation["observation/images/wrist"] = rng.integers(
            0, 256, size=(args.image_height, half_w, 3), dtype=np.uint8
        )
...
def main_websocket(args: ClientArgs) -> None:
    try:
        ...
        raise RuntimeError(
            "--transport websocket requires the optional 'websockets' package. Use the default TCP transport instead."
        ) from exc
    ...
    with websockets.sync.client.connect(uri, compression=None, max_size=None) as websocket:
        ...
        run_requests(
            ...
            rng,
            metadata,
            send=lambda obj: websocket.send(packb(obj)),
            recv=lambda: unpackb(websocket.recv()),
        )


def run_requests(args: ClientArgs, rng: np.random.Generator, metadata: dict[str, Any], *, send, recv) -> None:
    print(f"metadata={metadata}")
    expected_shape = (int(metadata.get("action_horizon", 15)), int(metadata.get("action_dim", args.action_dim)))
